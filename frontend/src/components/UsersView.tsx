...
export default function UsersView({
  ...
  const handleArchiveUser = async (id: string, name: string) => {
    if (
      !window.confirm(
        `Are you sure you want to archive ${name}? They will immediately lose access to the system.`
      )
    ) {
      return;
    }
    ...
    try {
      ...
      console.warn(
        "Backend archive failed, applying locally.",
        error
      );
    }
  };
  ...
  const handleRestoreUser = async (id: string, name: string) => {
    if (
      !window.confirm(
        `Restore account for ${name}? They will regain access.`
      )
    ) {
      return;
    }
    ...
    try {
      ...
      console.warn(
        "Backend restore failed, applying locally.",
        error
      );
    }
  };
  ...
  const handleDeleteUser = async (
    ...
    try {
      ...
      alert(
        `Account for ${name} has been permanently deleted.`
      );
    } catch (error) {
      console.error("Error deleting user:", error);

      alert(
        "Failed to permanently delete the account. Please try again."
      );
    }
  };
      ...
      <div className="max-w-7xl mx-auto space-y-6">
        ...
        <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-3xl p-6 shadow-sm gap-4">
          ...
          <div>
            ...
            <p className="text-xs text-slate-500 dark:text-slate-400 mt-0.5">
              Configure access control levels, personnel roles, and active accounts
            </p>
          </div>
        ...
        <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-3xl p-6 shadow-sm space-y-4">
          ...
          {isLoading ? (
            ...
            <p className="text-slate-600 dark:text-slate-400 text-xs italic py-8 text-center">
                  ...
                  {filteredRecords.map((record) => (
                    ...
                    >
                      ...
                      <td className="p-4">
                        ...
                        <span
                          className={`text-[11px] font-bold px-2.5 py-1 rounded-lg border ${getRoleBadgeStyle(
                            record.role
                          )}`}
                        >
                          {record.role}
                      ...
                      <td className="p-4">
                        ...
                        {record.username
                          ...
                          <span className="bg-emerald-100 dark:bg-emerald-500/10 text-emerald-700 dark:text-emerald-400 px-2 py-1 rounded text-xs font-bold border border-emerald-200 dark:border-emerald-500/20">
                            Low Risk (12)
                          </span>
                          ...
                          {mainTab === "Active" ? (
                            ...
                            <div className="flex justify-end gap-2">
                              ...
                              <button
                                ...
                                className="text-xs px-3 py-1.5 bg-rose-50 dark:bg-rose-950/40 text-rose-700 dark:text-rose-400 border border-rose-200 dark:border-rose-800/60 rounded-lg font-semibold cursor-pointer transition-colors hover:bg-rose-100 dark:hover:bg-rose-900/60"
                              >
                                Delete
                              </button>
