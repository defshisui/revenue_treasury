...
export default function HawkerAssociationApp({ onSubmitApplication }: Props) {
  ...
  // State to hold submitted applications fetched from the backend
  ...
  const fetchApplications = async (userEmail: string) => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        // Filter so citizens only see their own applications
        const userApps = data.filter((app: any) => app.submitterEmail === userEmail);

        // Sort by date (newest first) based on submissionDate or ID
      ...
      try {
        ...
        if (rawData) {
          ...
          if (fullname || email) {
            ...
            const userObj = { fullname, email, firstName, initials };
            setLoggedInUser(userObj);

            // Fetch records tied to this user
  ...
  const handleFinalSubmit = async () => {
    ...
    try {
      ...
      // Assuming this posts to your backend
      ...
      // Immediately fetch the fresh data from the backend to ensure sync
                    ...
                    <tr>
                      <td colSpan={5} className="bg-slate-50 py-20 text-center">
                        <div className="flex flex-col items-center justify-center space-y-1.5 text-slate-400">
                          ...
                          <p className="text-xs font-medium">No Applications Found</p>
                        </div>
                      </td>
                    </tr>
                  )}
