...
export default function RealPropertyApplication({ isCollapsed = false }: { isCollapsed?: boolean }) {
  ...
  const downloadRPTServiceQrCode = () => {
    if (!rptServiceQrCodeUrl) return;
    const link = document.createElement("a");
    link.href = rptServiceQrCodeUrl;
    link.download = `rpt-service-qrph-${rptServicePaymentApplication?.controlNumber || "payment"}.png`;
    document.body.appendChild(link);
    link.click();
    document.body.removeChild(link);
  };

  const downloadRPTAnnualQrCode = () => {
    if (!rptQrCodeUrl) return;
    const link = document.createElement("a");
    link.href = rptQrCodeUrl;
    link.download = `rpt-qrph-${Date.now()}.png`;
    document.body.appendChild(link);
    link.click();
    document.body.removeChild(link);
  };
  ...
  return (
    ...
    >
              ...
              <div className="p-6 sm:p-8 bg-slate-50/60 flex flex-col items-center">
                ...
                {rptQrCodeUrl && !rptQrPaid && (
                  <div className="w-full max-w-sm mt-6 flex flex-col items-center">
                    ...
                    <button type="button" onClick={downloadRPTAnnualQrCode} className="mt-2 w-full max-w-[280px] rounded-lg border border-slate-200 bg-white hover:bg-slate-50 py-2 text-[11px] font-bold text-slate-700 flex items-center justify-center gap-2 mx-auto"><span>⇩</span> Download QR Code</button>
                    </div>
      ...
      {isRPTServiceQrOpen && rptServicePaymentApplication && (
        <div className="fixed inset-0 z-[80] bg-slate-950/70 backdrop-blur-sm flex items-center justify-center p-3 sm:p-5 overflow-y-auto">
          <div className="bg-white rounded-[22px] w-full max-w-5xl shadow-2xl border border-slate-200 overflow-hidden my-auto">
            <div className="grid grid-cols-1 lg:grid-cols-[1.08fr_.92fr]">
              <div className="p-5 sm:p-7 lg:p-8"><div className="pr-0 lg:pr-7">
                <h2 className="text-xl sm:text-2xl font-black text-slate-900 leading-tight">RPT Service Payment</h2>
                <p className="text-lg sm:text-xl font-black text-slate-900 mt-0.5 break-all">({rptServicePaymentApplication.controlNumber || rptServicePaymentApplication.taxDeclarationNumber || "RPT Application"})</p>
                <p className="text-xs text-blue-700 font-bold mt-2">{rptServicePaymentApplication.service || "Real Property Tax Service"}</p>
                <div className="mt-4 text-sm text-slate-500 border-b border-slate-200 pb-5">Billed to <span className="font-bold text-slate-800">{rptServicePaymentApplication.applicantName || rptServicePaymentApplication.ownerName || currentUser?.fullname || "Taxpayer"}</span>{(rptServicePaymentApplication.email || currentUser?.email) && <>, {rptServicePaymentApplication.email || currentUser?.email}</>}</div>
                <div className="mt-6"><p className="text-4xl sm:text-5xl font-black text-emerald-600 tracking-tight">{formatCurrency(Number(rptServicePaymentApplication.paymentAmount || 0))}</p>
                  <div className="mt-7 space-y-3 text-sm"><div className="flex justify-between gap-4"><span className="text-slate-500">Subtotal</span><span className="font-bold text-slate-800">{formatCurrency(Number(rptServicePaymentApplication.paymentAmount || 0))}</span></div><div className="flex justify-between gap-4"><span className="text-slate-500">Fees</span><span className="font-bold text-slate-800">Free</span></div></div>
                  <div className="mt-5 pt-4 border-t border-slate-300 flex justify-between items-center"><span className="font-black text-slate-900">Total Due</span><span className="font-black text-lg text-slate-900">{formatCurrency(Number(rptServicePaymentApplication.paymentAmount || 0))}</span></div>
                </div>
                <div className="mt-8 rounded-xl border border-blue-200 bg-blue-50/70 p-4"><p className="text-[10px] font-black uppercase tracking-wider text-blue-800">Important Reminder</p><p className="text-[11px] leading-relaxed text-slate-600 mt-1">Please complete the payment while the QR Ph code is active. Check the amount before confirming in your banking or e-wallet app.</p></div>
              </div></div>
              <div className="p-5 sm:p-7 lg:p-8 bg-slate-50/60 lg:border-l border-slate-200 flex flex-col items-center">
                <div className="w-full max-w-[390px] rounded-2xl border border-slate-200 bg-white p-4 sm:p-5 shadow-sm text-center">
                  <p className="text-base font-black text-slate-900">Scan QR Ph code to pay</p><p className="text-[11px] text-slate-500 mt-1">Use your supported banking or e-wallet app.</p>
                  <div className="mt-3 h-9 rounded-xl bg-slate-50 border border-slate-100 flex items-center justify-center gap-3 overflow-hidden"><span className="text-[11px] font-black text-blue-700">GCash</span><span className="text-[11px] font-black text-emerald-600">maya</span><span className="text-[11px] font-black text-red-700">BPI</span><span className="text-[11px] font-black text-slate-900">GoTyme</span><span className="text-[10px] font-bold text-slate-500">and more⌄</span></div>
                  {isGeneratingRPTServiceQr && !rptServiceQrCodeUrl && <div className="mt-4 w-full aspect-square max-w-[270px] mx-auto rounded-xl border border-slate-200 bg-white flex flex-col items-center justify-center"><div className="h-9 w-9 border-4 border-slate-200 border-t-blue-700 rounded-full animate-spin mb-3"/><p className="text-xs font-bold text-blue-700">Generating QR Ph...</p><p className="text-[10px] text-slate-400 mt-1">Please wait</p></div>}
                  {!isGeneratingRPTServiceQr && rptServicePaymentNotice && !rptServiceQrCodeUrl && <div className="mt-4 rounded-xl border border-rose-200 bg-rose-50 p-4"><p className="text-[11px] font-bold text-rose-700">{rptServicePaymentNotice}</p><button type="button" onClick={() => void handlePayMongoRPTServiceQrPayment(rptServicePaymentApplication)} className="mt-3 px-4 py-2 bg-blue-900 hover:bg-blue-800 text-white rounded-lg text-[11px] font-bold">Generate QR Again</button></div>}
                  {rptServiceQrCodeUrl && !rptServiceQrPaid && <><div className="relative mt-4 w-fit mx-auto"><div className="absolute -top-3 left-1/2 -translate-x-1/2 z-10 rounded-full bg-emerald-500 text-white px-4 py-1 text-xs font-black shadow-md tabular-nums">{Math.floor(rptServiceQrSecondsRemaining / 60)}:{String(rptServiceQrSecondsRemaining % 60).padStart(2, "0")}</div><div className="bg-white border border-slate-200 rounded-xl p-3 shadow-md"><img src={rptServiceQrCodeUrl} alt="PayMongo Dynamic QR Ph payment code" className="w-56 h-56 object-contain"/></div></div><button type="button" onClick={downloadRPTServiceQrCode} className="mt-2 w-full max-w-[280px] rounded-lg border border-slate-200 bg-white hover:bg-slate-50 py-2 text-[11px] font-bold text-slate-700 flex items-center justify-center gap-2 mx-auto"><span>⇩</span> Download QR Code</button><div className={`mt-3 rounded-xl border px-4 py-3 ${rptServiceQrSecondsRemaining <= 30 ? "border-rose-200 bg-rose-50" : "border-blue-200 bg-blue-50"}`}><p className="text-[9px] font-semibold uppercase tracking-wider text-slate-500">QR code refreshes in</p><p className={`mt-0.5 text-xl font-black font-mono ${rptServiceQrSecondsRemaining <= 30 ? "text-rose-600" : "text-blue-700"}`}>{Math.floor(rptServiceQrSecondsRemaining / 60)}:{String(rptServiceQrSecondsRemaining % 60).padStart(2, "0")}</p><p className="text-[9px] text-slate-500 mt-0.5">A new QR code will be generated automatically when the timer expires.</p></div></>}
            ...
            <div className="px-5 sm:px-7 py-4 border-t border-slate-200 bg-white flex justify-end"><button type="button" onClick={closeRPTServicePayment} disabled={isGeneratingRPTServiceQr} className="px-6 py-2.5 rounded-xl bg-slate-100 hover:bg-slate-200 disabled:opacity-50 text-slate-700 font-bold text-xs">← Back to RPT Records</button></div>
          </div>
        </div>
      )}
