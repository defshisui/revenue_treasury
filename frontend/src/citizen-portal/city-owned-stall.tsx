...
export default function MarketStallApplication() {
    ...
    const handlePayMongoQrPayment = async (existingLeaseId?: string) => {
        ...
        const generatedLeaseId = existingLeaseId ||
  `LEASE-${new Date().getFullYear()}-${Date.now()}-${Math.floor(1000 + Math.random() * 9000)}`;
    ...
    // Dynamic QR Ph is refreshed every 5 minutes. The existing market lease is reused
    // so a QR refresh never creates a duplicate lease record.
    useEffect(() => {
        ...
        const timer = window.setInterval(() => {
            setQrSecondsRemaining((seconds) => {
                if (seconds <= 1) {
                    ...
                    setQrCodeUrl("");
                    void handlePayMongoQrPayment(qrLeaseId || undefined);
                    return 300;
                }
                return seconds - 1;
            });
    ...
    }, [isPaymentStep, qrCodeUrl, qrPaymentPaid, qrLeaseId]);

    // Poll the backend for webhook-confirmed payment status while the QR is open.
    useEffect(() => {
                                                    ...
                                                    <div>
                                                        ...
                                                        <div className="grid grid-cols-6 gap-1">
                                                            ...
                                                            {(() => {
                                                                ...
                                                                return (
                                                                    <button onClick={() => handleStallClick({ stallNum: 5, holder: st5.holder, availability: st5.availability, section: "Fish", type: "Permanent", size: "3 sqm", classification: "Regular", fee: "₱1.00" })} className={`${st5.availability === 'Occupied' ? 'bg-blue-700' : 'bg-blue-500'} hover:opacity-90 text-white text-[10px] font-bold h-10 rounded flex items-center justify-center relative shadow-sm cursor-pointer`}>
                                                                        5
                        ...
                        <div className="space-y-4 pt-4">
                            ...
                            {!isProcessingPayment && qrError && (
                                <div className="border border-rose-200 bg-rose-50 rounded-xl p-4 text-center">
                                    ...
                                    <button
                                        type="button"
                                        onClick={() => void handlePayMongoQrPayment(qrLeaseId || undefined)}
                            ...
                            {qrPaymentPaid && (
                                <div className="border border-emerald-200 bg-emerald-50 rounded-xl p-6 text-center">
                                    ...
                                    <p className="text-xs text-emerald-700 mt-1">PayMongo has confirmed your payment. Your lease payment status is being updated.</p>
                                </div>
                            )}
...
<div className="mt-4 w-full rounded-xl border border-sky-200 bg-sky-50 p-3 text-left">
