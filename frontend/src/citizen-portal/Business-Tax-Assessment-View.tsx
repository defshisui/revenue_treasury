...
export const BusinessTaxAssessmentView: React.FC<BusinessTaxAssessmentViewProps> = ({ isCollapsed = false }) => {
  ...
  const downloadPaymentQrCode = () => {
    ...
    link.href = qrCodeUrl;
    link.download = `business-tax-qrph-${paymentAssessment?.trackingNumber || 'payment'}.png`;
      ...
      {isModalOpen && (
        <div className="fixed inset-0 z-50 bg-black/60 backdrop-blur-xs flex items-center justify-center p-4">
          {isModalOpen === 'appointment' && (
            <form onSubmit={handleAppointmentSubmit} className="bg-white rounded-xl shadow-2xl border border-slate-200 w-full max-w-xl overflow-hidden">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-[1.08fr_.92fr]">
              ...
              <div className="p-5 sm:p-7 lg:p-8">
                <div className="pr-0 lg:pr-7">
                  ...
                  <p className="text-lg sm:text-xl font-black text-slate-900 mt-0.5 break-all">
                  ...
                  <div className="mt-4 text-sm text-slate-500 border-b border-slate-200 pb-5">
                    Billed to <span className="font-bold text-slate-800">{user?.fullname || paymentAssessment.businessOwner || "Business Taxpayer"}</span>
                    {user?.email && <>, {user.email}</>}
                  </div>

                  <div className="mt-6">
                    <p className="text-4xl sm:text-5xl font-black text-emerald-600 tracking-tight">
                      ₱{Number(paymentAssessment.computedFees?.total || 0).toLocaleString("en-PH", { minimumFractionDigits: 2, maximumFractionDigits: 2 })}
                    </p>
                  ...
                  <div className="mt-8 rounded-xl border border-blue-200 bg-blue-50/70 p-4">
                    ...
                    <p className="text-[11px] leading-relaxed text-slate-600 mt-1">
                      Please complete the payment while the QR Ph code is active. Check the amount before confirming in your banking or e-wallet app.
                    </p>
                  </div>
                </div>
              </div>
              ...
              <div className="p-5 sm:p-7 lg:p-8 bg-slate-50/60 lg:border-l border-slate-200 flex flex-col items-center">
                <div className="w-full max-w-[390px] rounded-2xl border border-slate-200 bg-white p-4 sm:p-5 shadow-sm text-center">
                  ...
                  {isProcessingPayment && !qrCodeUrl && (
                    <div className="mt-4 w-full aspect-square max-w-[270px] mx-auto rounded-xl border border-slate-200 bg-white flex flex-col items-center justify-center">
                      <div className="h-9 w-9 border-4 border-slate-200 border-t-blue-700 rounded-full animate-spin mb-3" />
                  ...
                  {!isProcessingPayment && qrError && !qrCodeUrl && (
                    <div className="mt-4 rounded-xl border border-rose-200 bg-rose-50 p-4">
                      <p className="text-[11px] font-bold text-rose-700">{qrError}</p>
                      <button type="button" onClick={() => void handlePayMongoBusinessTaxQrPayment(paymentAssessment)} className="mt-3 px-4 py-2 bg-blue-900 hover:bg-blue-800 text-white rounded-lg text-[11px] font-bold">Generate QR Again</button>
                    </div>
                  )}
                    ...
                    <>
                      <div className="relative mt-4 w-fit mx-auto">
                        <div className="absolute -top-3 left-1/2 -translate-x-1/2 z-10 rounded-full bg-emerald-500 text-white px-4 py-1 text-xs font-black shadow-md tabular-nums">
                          {Math.floor(qrSecondsRemaining / 60)}:{String(qrSecondsRemaining % 60).padStart(2, "0")}
                        </div>
                        <div className="bg-white border border-slate-200 rounded-xl p-3 shadow-md">
                          <img src={qrCodeUrl} alt="PayMongo Dynamic QR Ph payment code" className="w-56 h-56 object-contain" />
                        </div>
                      </div>
                      ...
                      <div className={`mt-3 rounded-xl border px-4 py-3 ${qrSecondsRemaining <= 30 ? "border-rose-200 bg-rose-50" : "border-blue-200 bg-blue-50"}`}>
                        <p className="text-[9px] font-semibold uppercase tracking-wider text-slate-500">QR code refreshes in</p>
                        <p className={`mt-0.5 text-xl font-black font-mono ${qrSecondsRemaining <= 30 ? "text-rose-600" : "text-blue-700"}`}>
                          {Math.floor(qrSecondsRemaining / 60)}:{String(qrSecondsRemaining % 60).padStart(2, "0")}
                        </p>
                ...
                <div className="w-full max-w-[390px] mt-4 flex items-center gap-2 text-[10px] text-slate-500">
                  <span className="text-base">♢</span>
                  <span>Your payment details are handled securely through PayMongo.</span>
                </div>
              </div>
            </div>

            <div className="px-5 sm:px-7 py-4 border-t border-slate-200 bg-white flex justify-end">
              <button type="button" onClick={closeBusinessTaxPayment} disabled={isProcessingPayment} className="px-6 py-2.5 rounded-xl bg-slate-100 hover:bg-slate-200 disabled:opacity-50 disabled:cursor-not-allowed text-slate-700 font-bold text-xs transition-colors">
                ← Back to Form
              </button>
            </div>
          </div>
